...
export const ReactSipProvider = ({
  ...
  const [activeMessages, setActiveMessages] = useState<{[key: string]: IMessage;}>({});
  ...
  const [availableMediaDevices, setAvailableMediaDevices] = useState<Array<any>>([]);
      ...
      init(domain, username, password, pcConfig) {
        try {
          ...
          openSIPSJS
            ...
            .on("changeActiveMessages", (sessions: any) => {
              setActiveMessages({
                ...sessions,
              });
            })
            .on(
              "newMSRPMessage",
              (data: { session: { _id: any }; message: any }) => {
            ...
            .on("changeActiveStream", (value: any | null) => {
